...
func TestSymbolIndex_NewSymbolIndex(t *testing.T) {
	a := TestAdapter[TestFileStoreItem]{}
	idx := index.NewSymbolIndex(&a)
	assertions.EqualUints(0, idx.Size(), t)
}

func TestSymbolIndex_Add(t *testing.T) {
	a := TestAdapter[adapter.FileStoreItem]{}
	idx := index.NewSymbolIndex(&a)
...
func TestSymbolIndex_Persist(t *testing.T) {
	...
	_ = os.Remove(a.GetIndexFilePath("TEST_PERSIST"))
	...
	text, _ := os.ReadFile(a.GetIndexFilePath("TEST_PERSIST"))
